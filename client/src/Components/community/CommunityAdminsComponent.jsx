...
export default function CommunityAdminsComponent({ communityId, isAdmin = false }) {
    const authToken = new AuthStorage().getAuth()
    const [state, setState] = useState(null)
    async function fetchCommunityAdmin() {
        try {
            const api = `${process.env.NEXT_PUBLIC_SERVER_URL}community/list-admins?id=${communityId}`
    ...
    async function removeAdmin(id) {
        const api = `${process.env.NEXT_PUBLIC_SERVER_URL}community/update-admins`
    ...
    return <div className="community-admins-component">
        {state && state.map((elem, index) => {
            ...
            return <div key={index}> <hr className="hr" />  <section className="each-member">
                ...
                {isAdmin &&
                    <button type="button" onClick={() => removeAdmin(id)}>Remove</button>}
            </section>
            </div>
        })}
    </div>
